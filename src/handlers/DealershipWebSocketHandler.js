...
class DynamicWebSocketHandler {
  constructor(ws, req, companyId) {
    ...
    this.responseDelay = 2500; // Increased from 1500ms to 2.5 seconds
  ...
  async processMessage(parsed) {
    ...
    // Natural conversation delay - wait longer for phone numbers
    const now = Date.now();
    const timeSinceLastResponse = now - this.lastResponseTime;
    const requiredDelay = this.conversationPhase === 'phone_number' ? 3000 : this.responseDelay;
    
    if (timeSinceLastResponse < requiredDelay) {
      await new Promise(resolve => setTimeout(resolve, requiredDelay - timeSinceLastResponse));
    }
  ...
  extractPhoneNumber(message) {
    ...
    let processedMessage = message.toLowerCase()
      ...
      .replace(/\bnine\b/g, '9')
      .replace(/\boh\b/g, '0')  // Handle "oh" as zero
    ...
    const matches = processedMessage.match(digitPattern);
    ...
    console.log(`📱 Phone extraction - Original: "${message}"`);
    console.log(`📱 Phone extraction - Processed: "${processedMessage}"`);
    console.log(`📱 Phone extraction - All digits: "${allDigits}" (length: ${allDigits.length})`);
    ...
    if (allDigits.length === 10) {
      ...
      // First, check if there's a clear area code + 7 digits pattern
      const areaCodeMatch = allDigits.match(/^(\d{3})(\d{7})/);
      if (areaCodeMatch) {
        return areaCodeMatch[1] + areaCodeMatch[2];
      }
      // Otherwise, take the last 10 digits
      return allDigits.slice(-10);
    } else if (allDigits.length === 9) {
      // Sometimes people forget to say the first digit
      console.log('⚠️ Only 9 digits found, might be missing area code digit');
    }
  ...
  async getResponse(userMessage) {
    ...
    switch (this.conversationPhase) {
      ...
      case 'phone_number':
        ...
        let phone = this.extractPhoneNumber(userMessage);
